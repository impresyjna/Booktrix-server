class Api::V1::FriendsController < ApplicationController
  ...
  def index
    ...
    case params[:list]
      when "index"
        render json: user.friends, each_serializer: FriendSerializer, status: 200, location: [:api, user]
      when "pending"
        render json: user.pending_friends, each_serializer: FriendSerializer, status: 200, location: [:api, user]
      when "requested"
        render json: user.requested_friends, each_serializer: FriendSerializer, status: 200, location: [:api, user]
      when "blocked"
        render json: user.blocked_friends, each_serializer: FriendSerializer, status: 200, location: [:api, user]
      else
  ...
  def create
    user = current_user
    friend = User.find_by(login: params[:friend])
    if user.login == params[:friend]
      ...
      render json: user.pending_friends, each_serializer: FriendSerializer, status: 201, location: [:api, user]
    end
  end
